...
export default function ScanRoute() {
  ...
  async function handlePickFromGallery() {
    ...
    try {
      ...
      if (!permission.granted) {
        setMessage("Photo library permission is required to select a QR image.");
        return;
      }
      ...
      try {
        const barcodes = await scanFromURLAsync(asset.uri, ["qr"]);
        const decodedValue = barcodes?.[0]?.data;
        if (decodedValue) {
          startAnalysis(decodedValue, "gallery");
          return;
        }
      } catch {
        // TODO: Keep a native image QR decoding fallback here if Android devices need broader support.
      }

      setMessage("TODO: Image QR decoding fallback is using a mock suspicious result for now.");
      startAnalysis("http://amaz0n-deals.ru/promo", "gallery", true);
    } finally {
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.actions}>
        ...
        <ScanAction icon="more-vertical" label="More" />
      </View>
...
function ScanAction({ icon, label, loading = false, onPress }) {
  return (
    <Pressable style={styles.smallAction} onPress={onPress}>
      <View style={styles.smallActionIcon}>
        {loading ? (
          <ActivityIndicator size="small" color="#6598D2" />
        ) : (
          <Feather name={icon} size={20} color="#6598D2" />
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  smallActionIcon: {
    ...
    borderRadius: 999,
    borderWidth: 1,
    borderColor: "rgba(110,150,210,0.25)",
