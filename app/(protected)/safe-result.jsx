...
export default function SafeResultRoute() {
  ...
  async function handleOpenLink() {
    if (!destinationUrl) {
      setLinkError("No destination URL is available for this QR code.");
      return;
    }
  ...
  async function handleShare() {
    try {
      await Share.share({
        title: "VAFPQR Scan Result",
        message: `VAFPQR Scan Result\nStatus: Safe\nDomain: ${domain}\nDestination: ${destinationUrl || "No destination URL"}`
      });
    } catch {
    ...
    <AppScreen scroll contentStyle={styles.screen}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.actionRow}>
          <OutlineButton label="Save Scan" icon="bookmark" onPress={handleSaveScan} style={styles.halfButton} />
          <OutlineButton label="Share" icon="share-2" onPress={handleShare} style={styles.halfButton} />
        </View>
